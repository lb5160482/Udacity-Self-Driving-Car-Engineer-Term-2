        ...
        if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    int ptsNum = ptsx.size();
                    Eigen::VectorXd ptsx_vec(ptsNum);
                    Eigen::VectorXd ptsy_vec(ptsNum);
                    
                    // car position as (0, 0) and transform waypoints from world coordinates to car coordinates
                    for (int i = 0; i < ptsNum; ++i) {
                        double dx = ptsx[i] - px;
                        double dy = ptsy[i] - py;
                        ptsx_vec(i) = dx * cos(-psi) - dy * sin(-psi);
                        ptsy_vec(i) =dx * sin(-psi) + dy * cos(-psi);
                    }
